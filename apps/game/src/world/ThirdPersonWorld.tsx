...
import { QualityBridge } from './qualityBridge'
...
export function ThirdPersonWorld({ scene, mood, active, paused, actionsRef, selected, reducedMotion }: { scene: SceneId | 'devin'; mood: Mood; active?: FounderId; paused: boolean; actionsRef: RefObject<PlayerActions | null>; selected: CharacterId; reducedMotion: boolean }) {
  ...
  // Quality tier (world/quality.ts): dpr cap, shadow map size and shadows on/off; R3F re-applies `dpr`/`shadows` on change.
  const quality = useQuality()
  return <Canvas shadows={quality.shadows ? 'soft' : false} dpr={quality.dpr} camera={{ position: [0, 3, 4.5], fov: 55, near: 0.1, far: 90 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.1 }} style={{ position: 'absolute', inset: 0, touchAction: 'none' }}>
    <QualityBridge />
    ...
    <directionalLight position={[-7, 10, 6]} color={M.sun} intensity={2.4} castShadow={quality.shadows} shadow-mapSize={[quality.shadowMapSize, quality.shadowMapSize]} shadow-camera-left={-16} shadow-camera-right={16} shadow-camera-top={16} shadow-camera-bottom={-16} shadow-normalBias={0.025} />
    ...
    <Suspense fallback={<Html center><p className="rounded bg-panel p-4 text-white">Loading physics and character…</p></Html>}>
      <Physics timeStep={1 / 60} paused={paused}>
        <WorldColliders scene={scene} />
        <CharacterController scene={scene} selected={selected} paused={paused} actionsRef={actionsRef} interaction={interaction} />
      </Physics>
    </Suspense>
  </Canvas>
}
