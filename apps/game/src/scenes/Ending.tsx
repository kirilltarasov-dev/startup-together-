...
export function Ending() {
  ...
  const win = verdict === 'STILL IN BUSINESS'
  ...
  return (
    <World scene="S3" mood={win ? 'win' : 'lose'}>
      <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl rounded-2xl border border-line bg-[#1A1B1E]/95 p-8 text-center">
          <div className="text-[10px] tracking-[0.4em] opacity-50">RUNWAY COMPLETE · DAY {g.day}</div>
          <motion.h1 initial={{ scale: 0.7 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 160, damping: 14 }} className={`text-6xl font-bold tracking-widest mt-2 ${win ? 'text-mint' : 'text-[#FF5A5F]'}`}>{verdict}</motion.h1>
