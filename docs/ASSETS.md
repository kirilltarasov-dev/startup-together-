...
## Environment Polish: Three Locations, One Lighting Rig, Smaller Downloads (VIS04, September 19, 2026)

No new external asset was acquired. The served CC0 environment files were re-encoded in place; every source download hash is retained in `apps/game/public/assets/environment/sources.json` (`sourceBytes`/`sourceSha256`) next to the served `bytes`/`sha256`, and `courtyard-tree.json` keeps the Blender export hash under `blenderExportSha256`.

| Served file | Before | After | Transformation |
| --- | --- | --- | --- |
| `courtyard-tree.glb` | 7,999,608 | 2,722,428 | `KHR_mesh_quantization` (u16 positions on a 0.07 mm grid via node scale/translation, int8 normals, u16 UVs), unused second UV set dropped on leaves/trunk, vertex reorder, `EXT_meshopt_compression` (vertex codec v0 / index codec v1 so the `three-stdlib` MeshoptDecoder that drei's `useGLTF` installs by default can decode it). 64,938 triangles and all nine <=512 px textures unchanged. |
| `courtyard.hdr` | 1,651,460 | 422,604 | 2x box-filtered downsample 1024x512 -> 512x256, RGBE RLE. Lighting/reflections only; never a visible background. |
| `brick-color.jpg`, `concrete-color.jpg` | 1,032,279 / 926,197 | 294,432 / 235,271 | JPEG quality 82, 1024x1024 unchanged |
| `brick-normal.jpg`, `concrete-normal.jpg` | 1,121,130 / 1,184,345 | 484,599 / 514,333 | JPEG quality 88 with 4:4:4 chroma (no subsampling of tangent-space data), PSNR ~35 dB against the download |
| `brick-roughness.jpg`, `concrete-roughness.jpg` | 391,526 / 475,799 | 57,558 / 65,774 | JPEG quality 80 |

Environment directory total (without manifests): 14,782,344 -> 4,796,999 bytes. The founder GLBs (16.1 MB) and the realism GLBs (2.2 MB, texture-dominated) were not touched. Reproduce from `apps/game`: `python3 scripts/shrink-environment-textures.py --overwrite` (Pillow only) and `node scripts/shrink-environment-assets.mjs --source <dir with the Blender export and the 1K HDR> --overwrite` (uses the already-installed `meshoptimizer` encoder; refuses compressed input and refuses to overwrite without the flag). `node --test environmentAssets.test.mjs` now also decodes the compressed tree with the runtime decoder and checks its bounds, the 512x256 HDR header and the retained source hashes.

Texture scale was measured from the maps themselves (row/column autocorrelation): `brick_wall_02` repeats every 20 courses, so one repeat is 1.5 m for a 7.5 cm course (was 2 m); `concrete_pavement` has 6 slabs per repeat, so 3.3 m gives 0.55 m slabs (was 2 m). Colour maps stay sRGB, normal/roughness maps linear. A 64 px generated tint mask (`sceneMaterials.ts`, `applySurfaceVariation`) modulates albedo +-11% and roughness +-0.12 over a 9 m period through a small `onBeforeCompile` patch shared by every textured surface, breaking visible tiling without extra downloads.

Scene composition (`SceneEnvironment.tsx`) now dresses the same collider layout three ways: S1 Puzl loft (mismatched tables, cable runs to a power strip, cans/mugs/pizza box, projector slide, rolling whiteboard, posters, scuffed path), S2 Debrecen apartment at 03:00 (2.56 m ceiling with beam, kitchen table/folding table/ironing board as desks, mattress and laundry line, kitchen counter and fridge in the unreachable side strip, noodle cups, one clamp lamp), S3 investor room (one bevelled conference table, symmetric glasses/notepads, plant, credenzas, wall screen, recessed downlights, white walls over a dark floor). `sceneLighting.tsx` holds the per-location rig (sun, hemisphere, HDR intensity, sky sun position, fog, exposure, practical point/spot lights in candela, contact shadows) and drives the host canvases' existing sun/sky/fog/exposure imperatively; there is still exactly one ACES tone-mapping pass and no post-processing composer. No night HDR was acquired; the night look uses the daylight HDR at 6% plus a below-horizon sky sun, a cold window spot and warm practicals. Evidence: `TEST_URL=http://127.0.0.1:4202 LABEL=after node scripts/environment-polish.browser.mjs` writes identical-camera screenshots (spawn, desk, side, three face close-ups, narrow) and per-type transfer bytes under `node_modules/.cache/runway-environment-polish/`; `window.__runwayCamera(view)` and `window.__runwayLighting()` are read-only evidence hooks exposed by `sceneLighting.tsx` (they move the camera / report the rig, never game state).
